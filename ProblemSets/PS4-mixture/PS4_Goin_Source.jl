...
function practice_quadrature()
    ...
    nodes, weights = lgwt(7, -4, 4) #7 grid points, integral from -4 to 4
...
function variance_quadrature()
    ...
    #comment on how good the approx is. 
...
function mixed_logit_quad(theta, X, Z, y, R)
    ...
    #loop through gridpoints to do summation via loop
...
function optimize_mlogit(X, Z, y)
    ...
    # initialize the TwiceDifferentiable object
...
function allwrap()
    ...
    println("Same interpretation as PS3, this time it is large and positive
    with a t-stat of > 100")
